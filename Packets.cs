...
// Last generated on Tue Dec 21 22:05:29 2010
using System;
...
namespace SpacecraftGT
{
    public abstract partial class Packet
    {
        ...
        public static Packet Load(byte[] data)
        {
			...
			switch (data[0]) {
				...
				case 0x33: { //MapChunk
					var Packet = Output as MapChunkPacket;
					Packet = new MapChunkPacket();
					Packet.ID = GetNextByte(data, ref BufferPos);
					Packet.X = GetNextInt(data, ref BufferPos);
					Packet.Y = GetNextShort(data, ref BufferPos);
					Packet.Z = GetNextInt(data, ref BufferPos);
					Packet.Size_X = GetNextByte(data, ref BufferPos);
					Packet.Size_Y = GetNextByte(data, ref BufferPos);
					Packet.Size_Z = GetNextByte(data, ref BufferPos);
					Packet.Size = GetNextInt(data, ref BufferPos);
					Packet.Data = GetNextByteArray(data, ref BufferPos);
					break; }
	...
	public class MapChunkPacket : Packet {
		...
		public override byte[] Save() { 
			...
			Build.Append( IPAddress.HostToNetworkOrder( Y ).ToBytes() );
			Build.Append( IPAddress.HostToNetworkOrder( Z ).ToBytes() );
			Build.Append(  Size_X.ToBytes() );
			Build.Append(  Size_Y.ToBytes() );
			Build.Append(  Size_Z.ToBytes() );
			Build.Append( IPAddress.HostToNetworkOrder( Size ).ToBytes() );
			Build.Append(  Data.ToBytes() );
			return Build.ToArray();
		}
	}
